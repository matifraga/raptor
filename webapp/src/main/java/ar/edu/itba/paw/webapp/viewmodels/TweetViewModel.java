...
public class TweetViewModel {
	...
	public TweetViewModel(Tweet tweet) {
		if (tweet.isRetweet()) {
			...
			this.retweetedBy = new StringBuilder(tweet.getOwner().getFirstName())
					.append(" ").append(tweet.getOwner().getLastName()).toString();
    ...
    public static List<TweetViewModel> transform(List<Tweet> tweetList) {
        ...
        for (Tweet tweet : tweetList) 
            tweetMList.add(transformTweet(tweet)); 
	...
	private String parseURL(String s) {
		...
		for (int i = 0; i < parts.length; i++)
			try {
				...
				parts[i] = new StringBuilder("<a href=\"").append(url)
						.append("\">").append(url).append("</a> ").toString();
	...
	private String parseHashtags(String s) {
		...
		while (matcher.find()) {
			...
			StringBuilder searchHTML = new StringBuilder(
					"<a href='search?searchText=%23").append(search)
	...
	private String parseUsers(String s) {
		...
		while (matcher.find()) {
			...
			StringBuilder userHTML = new StringBuilder("<a href='user/")
					.append(rawName).append("'>").append(result).append("</a>");
